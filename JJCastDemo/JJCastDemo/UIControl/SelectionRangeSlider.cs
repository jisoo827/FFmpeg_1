    ...
    public partial class SelectionRangeSlider : UserControl
    {
        ...
        void SelectionRangeSlider_Paint(object sender, PaintEventArgs e)
        {
            ...
            Rectangle selectionRect = new Rectangle(
                ...
                Height);
            SolidBrush blueBrush = new SolidBrush(Color.FromArgb(128, ((int)(((byte)(0)))), ((int)(((byte)(120)))), ((int)(((byte)(215))))));
            e.Graphics.FillRectangle(blueBrush, selectionRect);
        ...
        void SelectionRangeSlider_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (movingMode == MovingMode.MovingValue)
                Value = Math.Max(Math.Min(pointedValue, Max - 1), 1);
            else if (movingMode == MovingMode.MovingMin)
            {
                SelectedMin = Math.Max(pointedValue, 0);
                SelectedMin = SelectedMin >= SelectedMax ? SelectedMax - 1 : SelectedMin;
            }
            else if (movingMode == MovingMode.MovingMax)
            {
                SelectedMax = Math.Min(pointedValue, Max);
                SelectedMax = SelectedMax <= SelectedMin ? SelectedMin + 1 : SelectedMax;
            }
        }
